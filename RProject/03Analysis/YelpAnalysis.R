...
p1 <- ggplot(subset(businesses, CITY == "Phoenix"), aes(x=LONGITUDE, y=LATITUDE, color = BUSINESS_OPEN)) + geom_point() + ggtitle("Phoenix") + theme(legend.position="none")
p2 <- ggplot(subset(businesses, CITY == "Madison"), aes(x=LONGITUDE, y=LATITUDE, color = BUSINESS_OPEN)) + geom_point() + ggtitle("Madison") + theme(legend.position="none")
