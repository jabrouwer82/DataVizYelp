...
def write_table_to_batabase(table):
  if table == 'business':
    for business_id, business in businesses.items():
      insert = BUSINESS_INSERT.format(business['business_id'],
                                      business['business_id_str'],
  ...
  elif table in relation_tables:
    return RELATION_INSERT.format(table,
                                  values[0],
                                  values[1],
                                  values[2],
                                  values[3])
  elif table == 'neighborhood':
  ...
  elif table == 'elite_year':
    return ELITE_YEAR_INSERT.format(values[0],
                             values[1])
  elif table == 'compliment':
...
def write_attribute_businesses():
